...
namespace ProtoMolAddon {
  ...
  public:
    ...
    virtual double GetNNPotential(const Vector3D& pos, const array<int, 3>& offset=array<int, 3>()) const = 0;
    virtual double GetNNVoltage(double time, int offset=0) const = 0;
    virtual const Vector3D& GetDx() = 0;
    virtual void GetFraction(const Vector3D &pos, array<double, 3>& f) = 0;
