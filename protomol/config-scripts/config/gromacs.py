...
def add_vars(vars):
    vars.AddVariables(
        BoolVariable('fah', 'Set to 1 to build for Folding@home', 1),
        )
...
def configure(conf):
    ...
    if not int(env.get('fah', 1)):
        try:
            env.ParseConfig('pkg-config --libs --cflags libgmx libmd')
            found = True
        except: pass
    ...
    for lib in ['gmx', 'md']: config.require_lib(conf, lib)
